pipeline {
  ...
  stages {
    ...
    stage('Docker Build') {
      steps {
        sh '/usr/bin/docker build -t anilbunny/bank-customer-service:latest .'
      }
    }
    stage('Push image') {
      steps {
        ...
        //withDockerRegistry(credentialsId: 'dvishalanand', url: 'https://118463809662.dkr.ecr.ap-south-1.amazonaws.com/visanand') {
          sh '/usr/bin/docker tag bank-customer-service anilbunny/bank-customer-service:latest'
          sh '/usr/bin/docker push anilbunny/bank-customer-service:latest'
